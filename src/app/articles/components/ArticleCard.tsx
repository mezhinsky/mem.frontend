...
export function ArticleCard({ id, title, description, image, date }: ArticleCardProps) {
  return (
    <article
      key={id}
    ...
    >
      ...
      <div className="p-4 flex flex-col h-full">
        ...
        {date && <p className="text-xs text-gray-400 mt-auto">{new Date(date).toLocaleDateString()}</p>}
      </div>
    </article>
  );
}